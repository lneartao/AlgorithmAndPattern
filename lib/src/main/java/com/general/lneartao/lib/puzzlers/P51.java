...
/**
 * 修正版本的Point
 * 尽管懒性加载可以修正这个问题，但是对于让一个值类去扩展另一个值类，并且在其中添加一个会对equals比较丰富产生影响的域的这种做法仍旧不是一个好主意
 * 你无法在超类和子类上都提供一个基于值的equals方法，而同时又不违反Object.equals方法的通用约定，或是只不消除在超类和子类之间进行有实际意义的比较操作的可能性
 */
class ModifiedPoint {
    protected final int x, y;
    private String name;
